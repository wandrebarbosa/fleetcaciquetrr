...
import { Veiculo, TipoManutencao } from '@/types/fleet';
...
const MaintenanceModal: React.FC<MaintenanceModalProps> = ({ veiculo, open, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addManutencao({
      ...
      tipoManutencao: tipo,
      grupoServicos: selectedServicos,
      kmRegistrado: Number(kmAtual),
      solicitante: solicitante.trim(),
      fornecedores: fornecedores.trim(),
      dataInicio: dataParada || new Date().toISOString(),
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Select value={tipo} onValueChange={(v) => { setTipo(v as TipoManutencao); setSelectedServicos([]); }}>
            ...
            <div className="mt-2 space-y-2 max-h-36 overflow-y-auto border rounded-md p-3">
              {filteredServicos.map(s => (
                <label key={s.id} className="flex items-center gap-2 text-sm cursor-pointer">
                  <Checkbox
                    checked={selectedServicos.includes(s.nome)}
                    onCheckedChange={() => toggleServico(s.nome)}
                  />
                  {s.nome}
                </label>
              ))}
          ...
          <div>
            ...
            <Input value={fornecedores} onChange={e => setFornecedores(e.target.value)} placeholder="Separados por vírgula" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label>Data/Hora Parada</Label>
              <Input type="datetime-local" value={dataParada} onChange={e => setDataParada(e.target.value)} required />
            </div>
            <div>
              <Label>Previsão Retorno</Label>
              <Input type="datetime-local" value={previsaoRetorno} onChange={e => setPrevisaoRetorno(e.target.value)} />
            </div>
          </div>
